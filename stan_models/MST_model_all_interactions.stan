...
parameters{  
    ...
    real bT_heat;             // effect of long-term warming (heated-ancestral communities) on activation energy
...
transformed parameters{
    ...
    mu = ln_r0 + ln_r0_heat*ancest + ln_biomass + bT*temp + bT_heat*temp_inter;
}
model{
...
generated quantities{
    ...
    for(i in 1:N){
        temp_cor_mu[i] = ln_rate[i] - log(exp(bT*temp[i])) - log(exp(bT_heat*temp_inter[i]));
        temp_cor_sim[i] = normal_rng(ln_rate[i] - log(exp(bT*temp[i])) - log(exp(bT_heat*temp_inter[i])), sigma);
